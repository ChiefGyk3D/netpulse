...
INFLUXDB_TOKEN="netpulse-token-change-me"
...
print_help() {
    ...
    echo "  --influxdb-token T   InfluxDB token"
    ...
    echo ""
    echo "Examples:"
    echo "  $0 --local                           # Local install with systemd"
    echo "  $0 --docker                          # Docker Compose (build locally)"
    echo "  $0 --docker-ghcr                     # Docker with GHCR image"
    echo "  $0 --local --interval 15             # Run speedtest every 15 minutes"
